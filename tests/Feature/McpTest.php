...
covers(AppServer::class, LaunchApp::class, MainApp::class, AppServiceProvider::class, FortifyServiceProvider::class, EnsureSecurityHeaders::class, User::class);
...
it('directly configures appMeta and handles request on main app resource', function (): void {
    ...
    $resource = App::make(MainApp::class);
    $meta = $resource->appMeta();
    $metaArray = $meta->toArray();

    $prefersBorder = $metaArray['prefersBorder'] ?? null;
    $csp = $metaArray['csp'] ?? null;

    expect($prefersBorder)->toBeFalse()
        ->and($csp)->toBe([
...
it('deduplicates identical api and frontend urls in connectDomains', function (): void {
    Config::set('app.url', 'https://same.test');
    Config::set('app.frontend_url', 'https://same.test');

    $resource = App::make(MainApp::class);
    $meta = $resource->appMeta()->toArray();
    $csp = $meta['csp'] ?? null;

    expect($csp)->toBe([
        'connectDomains' => ['https://same.test'],
        'resourceDomains' => ['https://same.test'],
    ]);
});
