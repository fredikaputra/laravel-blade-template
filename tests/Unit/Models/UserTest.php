...
covers(User::class, AppServiceProvider::class, FortifyServiceProvider::class);
...
it('broadcasts events when created, updated, and deleted', function (): void {
    ...
    Event::assertDispatched(function (BroadcastableModelEventOccurred $event) use ($user): bool {
        $channels = $event->broadcastOn();
        $channel = $channels[0] ?? null;

        return $event->broadcastAs() === 'UserCreated'
            && $event->model->is($user)
    ...
    Event::assertDispatched(fn (BroadcastableModelEventOccurred $event): bool => $event->broadcastAs() === 'UserUpdated'
        && $event->model instanceof User
        && $event->model->name === 'Alice Wonder');

    $user->delete();

    Event::assertDispatched(fn (BroadcastableModelEventOccurred $event): bool => $event->broadcastAs() === 'UserDeleted'
        && $event->model->is($user));
});
