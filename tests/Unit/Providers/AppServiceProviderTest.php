...
covers(AppServiceProvider::class, FortifyServiceProvider::class, User::class);
...
it('registers and boots application service provider', function (): void {
    ...
    expect($provider->isDeferred())->toBeFalse()
        ->and(new Illuminate\Support\Collection(DevCommands::commands())->firstWhere('name', 'nightwatch')['command'] ?? null)->toBe('php artisan nightwatch:agent')
        ->and(new Illuminate\Support\Collection(DevCommands::commands())->firstWhere('name', 'schedule')['command'] ?? null)->toBe('php artisan schedule:work')
        ->and(new Illuminate\Support\Collection(DevCommands::commands())->firstWhere('name', 'pulse')['command'] ?? null)->toBe('php artisan pulse:check')
...
it('builds password reset links using the frontend URL', function (): void {
    ...
    expect($callback($user, 'sometoken'))
...
it('rejects a non-user notifiable when building password reset links', function (): void {
    ...
    expect(fn (): string => $callback(new Illuminate\Support\Collection([]), 'sometoken'))
